...
class AuditReplyCommentEdit extends BaseController
{
    ...
    public function index($empno)
    {
        ...
        echo view('AuditRepliedCommentEditing/ongoingAssignedRepliedEditingAudit',$data);
        echo view('templates/footer'); 
    }

    public function ongoingAssignedReplyCommentEditAllAudit(){

        $audityear = $this->request->getPost("cId");
        $empno = $this->request->getPost("empno");
        $auditcreationData = $this->auditfieldwork->assignedAuditReplyCommentEditAllAudit("auditcreation",array("audityear"=>$audityear),array("empno"=>$empno));
        ...
        foreach($auditcreationData as $auditcreateddetailall){
            ...
            <td><a href='/Development/development/public/OngoingReplyEditingAllComments/$auditcreateddetailall->auditid' class='btn btn-secondary'>Comments</a></td>
            </tr>";
        }
    ...
    public function OngoingReplyEditingAllComments($auditid){
        ...
        echo view('templates/headerfieldwork');
        echo view('AuditRepliedCommentEditing/ongoingReplyEditingAllComments',$data);
        echo view('templates/footer'); 
    }
    
    public function OngoingCommentReplyCommentEditing($comment_id){
        
        $comments = new CommentsModel();
        $commentsdetails = $comments->where('comment_id',$comment_id)->first();

        if($commentsdetails["status"] == "Comment Rejected to Enterer"){

            $newdata= [
                'status'=> 'Replied Comment Editing',
        ...
        if($this->request->getMethod()== 'post'){
            ...
            if(! $this->validate($rules)){
                ...
                echo view('templates/headerfieldwork');
                echo view('AuditRepliedCommentEditing/commentsReplyEdiitng',$data);
                echo view('AuditRepliedCommentEditing/managementReplyingEditing');
                echo view('templates/footer'); 
                ...
                return redirect()->to('http://localhost/Development/development/public/OngoingCommentReplyCommentEditing/'.$comment_id);
            }
    ...
    public function CommentReplyEditSubmitRview($comment_id){

        $comments = new CommentsModel();
        $commentsdetails = $comments->where('comment_id',$comment_id)->first();
        $newdata= [
            'status'=> 'Submitted the edited comment to review',
            'statusupdate_at'=> date("Y-m-d H:i:s")
        ];
        if($commentsdetails["status"] == "Replied Comment Editing"){
                $comments->where('comment_id',$comment_id)->set($newdata)->update();

                $commentlog = new CommentsLogModel();
                $commentlogdata = [
                    'comment_id'=> $comment_id,
                    'status'=> 'Submitted the replied comment edited review',
                    'updated_by' => session()->get('EmpNo'),
                    'updated_at' => date("Y-m-d H:i:s"),  
                    ];
                $commentlog->save($commentlogdata);
                $session = session();
                $session->setFlashdata('success','Comment was submitted  to reviewer with management reply ');
                return redirect()->to('http://localhost/Development/development/public/OngoingCommentReplyCommentEditing/'.$comment_id);
        
        }else if($commentsdetails["status"] == "Submitted the edited comment to review"){
            
                $session = session();
                $session->setFlashdata('Reject','Comment was already submitted back to enterer ');
                return redirect()->to('http://localhost/Development/development/public/OngoingCommentReplyCommentEditing/'.$comment_id);
            ...
            $session = session();
            $session->setFlashdata('Reject','Comment and reply were alredy accepted');
            return redirect()->to('http://localhost/Development/development/public/OngoingCommentReplyCommentEditing/'.$comment_id);

        }else if($commentsdetails["status"] == "Reviewing Management Reply"){
            $session = session();
            $session->setFlashdata('Reject','Comment and management reply is being reviewed by the reviewer ');
            return redirect()->to('http://localhost/Development/development/public/OngoingCommentReplyCommentEditing/'.$comment_id);

        }else if($commentsdetails["status"] == "Recieved from branch"){
            $session = session();
            $session->setFlashdata('Reject','Managment reply received from the branch, still reviwer did not review');
            return redirect()->to('http://localhost/Development/development/public/OngoingCommentReplyCommentEditing/'.$comment_id);
        }
